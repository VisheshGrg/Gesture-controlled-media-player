...
while True:   
    ...
    end_row, end_col = int(height), int(width*0.7)
    ...
    face_mask = create_mask_face(frame)
    threshImg = threshold(mask)
    threshImgFace = threshold(face_mask)
    contours = find_contours(mask)
    contours_Face = find_contours(face_mask)
    frame = cv2.drawContours(frame, contours, -1, (255, 0, 0), 2)  # drawing all contours
    frame = cv2.drawContours(frame, contours_Face, -1, (255, 0, 0), 2) 
    max_cntr = max_contour(contours)  # finding maximum contour of the thresholded area
    max_cntr_face = max_contour(contours_Face)

    (centroid_x, centroid_y) = centroid(max_cntr)
    (centroid_face_x, centroid_face_y) = centroid(max_cntr_face)

    if(centroid_face_x !=-1 and centroid_face_y !=-1) :
        frame = cv2.circle(frame, (centroid_face_x,centroid_face_y), 5, (255,0,0), -1)

        face_detected = cv2.contourArea(max_cntr_face) >= 2000
        if face_detected:
            if lastMotion==True:
